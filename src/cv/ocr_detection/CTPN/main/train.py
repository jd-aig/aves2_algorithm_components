...
'input'
tf.app.flags.DEFINE_string('restore_checkpoint_path', '', '')
tf.app.flags.DEFINE_string('pretrained_model_path_vgg', '', '')
...
def main(argv=None):
    ...
    pretrained_model_path_vgg = FLAGS.pretrained_model_path_vgg + 'vgg_16.ckpt'
    print(pretrained_model_path_vgg)
    
    if pretrained_model_path_vgg is not None:
        variable_restore_op = slim.assign_from_checkpoint_fn(pretrained_model_path_vgg,
    ...
    with tf.Session(config=config) as sess:
        if FLAGS.restore:
            ckpt = tf.train.latest_checkpoint(FLAGS.restore_checkpoint_path)
        ...
        data_generator = data_provider.get_batch(num_workers=FLAGS.num_readers,data_folder = FLAGS.data_folder)
        start = time.time()
        '''
        print(start)
        print(restore_step)
        ...
        for step in range(restore_step, FLAGS.max_steps):
            ...
            if (step + 1) % FLAGS.save_checkpoint_steps == 0:
                ...
                filename = os.path.join(FLAGS.export_checkpoint_path, filename)
