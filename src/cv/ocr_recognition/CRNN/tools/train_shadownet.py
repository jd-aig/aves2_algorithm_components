...
def init_args():
    ...
    parser.add_argument('-t', '--tboard_save_dir', type=str,
                         help='Directory where tboard file to be stored')
    parser.add_argument('-m', '--model_save_dir', type=str,
                         help='Directory where ckpt file to be stored')
...
def compute_net_gradients(images, iabels, net, optimizer=None, is_net_first_initialized=False):
    """
...
def train_shadownet(dataset_dir, weights_path, char_dict_path, ord_map_dict_path, tboard_save_dir, model_save_dir,need_decode=False):
    """
    ...
    #print_op = tf.print(train_labels)
    print_op = tf.no_op()
    import json
    num_classes = len(json.load(open(char_dict_path)).keys())+1
    ...
    with tf.device('/gpu:0'):
        ...
        #train_inference_ret = tf.Print(train_inference_ret,[train_inference_ret])
        #val_inference_ret = tf.Print(val_inference_ret,[val_inference_ret])
        #print_op = tf.print(tf.math.equal(train_inference_ret,val_inference_ret))
        print_op = tf.no_op()
        with tf.control_dependencies([print_op]):
            ...
            val_decoded, val_log_prob = tf.nn.ctc_beam_search_decoder(
                val_inference_ret,
                CFG.ARCH.SEQ_LENGTH * np.ones(CFG.TRAIN.VAL_BATCH_SIZE),
                merge_repeated=False
            )
    ...
    with sess.as_default():
        ...
        while epoch < train_epochs:
            ...
            if need_decode and epoch % 100 == 0:
                
                # train part
                _, train_ctc_loss_value, train_seq_dist_value, \
                    train_predictions, train_labels_sparse, merge_summary_value,images_paths = sess.run(
                     [optimizer, train_ctc_loss, train_sequence_dist,
                      train_decoded, train_labels, merge_summary_op,train_images_paths],feed_dict={is_training_flag: True})
                ...
                if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                    logger.info('Epoch_Train: {:d} cost= {:5f} seq distance= {:5f} train accuracy= {:5f}'.format(
                        epoch + 1, train_ctc_loss_value, train_seq_dist_value, avg_train_accuracy))
               
                # validation part
                val_ctc_loss_value, val_seq_dist_value, \
                    val_predictions, val_labels_sparse, images_paths= sess.run(
                     [val_ctc_loss, val_sequence_dist, val_decoded, val_labels, val_images_paths],feed_dict={is_training_flag: False})
                val_labels_str = decoder.sparse_tensor_to_str(val_labels_sparse)
                #print(val_labels_str)
                #print(images_paths)
                ...
                if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                    logger.info('Epoch_Val: {:d} cost= {:5f} seq distance= {:5f} val accuracy= {:5f}'.format(
                        epoch + 1, val_ctc_loss_value, val_seq_dist_value, avg_val_accuracy))
                if avg_val_accuracy >= max_val_accu:
                    logger.info('saving ckpt')
                    saver.save(sess=sess, save_path=model_save_path, global_step=epoch)
                    max_val_accu = avg_val_accuracy
            else:
                _, train_ctc_loss_value, merge_summary_value = sess.run(
                    [optimizer, train_ctc_loss, merge_summary_op],feed_dict={is_training_flag: True})

                if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                    logger.info('Epoch_Train: {:d} cost= {:5f}'.format(epoch + 1, train_ctc_loss_value))
...
if __name__ == '__main__':
    ...
    char_dict_path = os.path.join(args.char_dict_path,'char_dict.json')
    ord_map_dict_path = os.path.join(args.ord_map_dict_path,'ord_map.json')
    if args.weights_path is not None:
        weights_path = tf.train.latest_checkpoint(args.weights_path)
    else :
        weights_path = None
    print(char_dict_path)
    print(ord_map_dict_path)
    print(weights_path)
    train_shadownet(
        ...
        tboard_save_dir = args.tboard_save_dir,
