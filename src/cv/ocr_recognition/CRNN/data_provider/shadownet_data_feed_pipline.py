...
class CrnnDataProducer(object):
    ...
    def __init__(self, dataset_dir, char_dict_path=None, ord_map_dict_path=None,
        ...
        self._train_annotation_file_path = ops.join(dataset_dir, 'Train/sample.txt')
        self._test_annotation_file_path = ops.join(dataset_dir, 'Test/sample.txt')
        self._val_annotation_file_path = ops.join(dataset_dir, 'Validation/sample.txt')
        ...
        if char_dict_path is None or ord_map_dict_path is None:
            #os.makedirs('./data/new_char_dict')
            os.makedirs('./data/new_char_dict', exist_ok=True)
            self._char_dict_path = ops.join('./data/new_char_dict', 'char_dict.json')
            self._ord_map_dict_path = ops.join('./data/new_char_dict', 'ord_map.json')
            self._generate_char_dict()
        '''
        os.makedirs('./data/new_char_dict', exist_ok=True)
        self._char_dict_path = ops.join(char_dict_path, 'char_dict.json')
        self._ord_map_dict_path = ops.join(ord_map_dict_path, 'ord_map.json')
        self._generate_char_dict()
    ...
    def generate_tfrecords(self, save_dir):
        ...
        t_start = time.time()
        """
        print(self._train_sample_infos)
        print(self._val_sample_infos)
        print(self._test_sample_infos)
        """
    ...
    def _init_dataset_sample_info(self):
        ...
        with open(self._train_annotation_file_path, 'r') as file:
            for line in tqdm.tqdm(file, total=num_lines):
                [image_name,label_index] = line.split(', ')
                label_index = label_index.split('"')[1]
                self._lexicon_list.append(label_index)     
                #image_name, label_index = line.rstrip(',').rstrip('\n').split(' ')
                image_path = ops.join(self._dataset_dir + 'Train/images/', image_name)
                try:
                    ops.exists(image_path)
        ...
        with open(self._val_annotation_file_path, 'r') as file:
            for line in tqdm.tqdm(file, total=num_lines):
                ...
                image_path = ops.join(self._dataset_dir + 'Validation/images/', image_name)
                #label_index = int(label_index)
                try :
                    ops.exists(image_path)
        ...
        with open(self._test_annotation_file_path, 'r') as file:
            for line in tqdm.tqdm(file, total=num_lines):
                ...
                self._lexicon_list.append(label_index)
                #image_name, label_index = line.rstrip('\r').rstrip('\n').split(' ')
                image_path = ops.join(self._dataset_dir + 'Test/images/', image_name)
                #label_index = int(label_index)
                try :
                    ops.exists(image_path)
