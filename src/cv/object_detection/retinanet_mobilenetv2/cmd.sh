ARGS=`getopt -o : -l "help,output_path:,tfrecords_dir:,pretrainmodel_dir:,learning_rate:,epochs:,batch_size:,alpha_fr:,gamma_fr:" -- "$@"`
eval set -- "${ARGS}"
...
CONFIG_FILE="ssd_mobilenet_v2_coco_focal_loss.config"
...
alpha_fr_val=${run_map['alpha_fr']}
gamma_fr_val=${run_map['gamma_fr']}
...
sed -i "s:${alpha_fr}:${alpha_fr_val}:g" ${CONFIG_FILE}
sed -i "s:${gamma_fr}:${gamma_fr_val}:g" ${CONFIG_FILE}

echo "alpha_fr and gamma_fr is:"
echo ${alpha_fr_val} 
echo ${gamma_fr_val}
