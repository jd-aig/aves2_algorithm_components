...
class ObjectDetectionEvaluation(object):
  ...
  def __init__(self,
               ...
               categories,
               matching_iou_threshold=0.5,
               nms_iou_threshold=0.5,#1.0,
               nms_max_output_boxes=50,#10000,
               use_weighted_mean_ap=False,
               label_id_offset=0,
               group_of_weight=0.0):
  ...
  def evaluate(self):
    ...
    """
    precisions_score_threshold = 0.0
    ...
    for class_index in range(self.num_class):
      ...
      #print('label: {}'.format(class_index))
      precision, recall = metrics.compute_precision_recall(
          scores, tp_fp_labels, self.num_gt_instances_per_class[class_index])
      #print("precision, recall = ",len(precision),len(recall),precision, recall)
      ...
      if idx < 0:
          idx = 0
      print('len(tf_fp_labels) = ', len(tp_fp_labels))
      print("len(precision) = ",len(precision))
      print("len(recall) = ",len(recall))
      print('len(scores) = ', len(scores))
      print("idx = ",idx)
      sorted_indices = sorted_indices[0:idx+1]
      true_positive_labels = tp_fp_labels[sorted_indices]
      false_positive_labels = (true_positive_labels <= 0).astype(float)
      cum_true_positives = np.cumsum(true_positive_labels)
      cum_false_positives = np.cumsum(false_positive_labels)
      print("true_positives:",cum_true_positives[-1])
      print("false_positives:",cum_false_positives[-1])
      print("self.num_gt_instances_per_class[class_index] = ",self.num_gt_instances_per_class[class_index])
      
      if len(precision)==0:
        pass
      else:
        precisions_score_threshold = precisions_score_threshold + precision[idx]
        recall_score_threshold = recall_score_threshold + recall[idx]
      
      print("precisions_score_threshold = ",precisions_score_threshold)
      print("recall_score_threshold = ",recall_score_threshold)
      print("-------------------------------------------------------")
      ...
      #print(self._categories[class_index])
      #print(self._categories[class_index]["name"])
      ap_dict[str(self._categories[class_index]["name"])]=average_precision
      #ap_dict[class_index]=average_precision
      #print('precisions per class label:',precision)
      #print('recall: per class label',recall)
    ...
    #print("precisions_per_class = ",self.precisions_per_class)
    #print("recalls_per_class = ",self.recalls_per_class)

    precisions_score_threshold = precisions_score_threshold/self.num_class
    recall_score_threshold = recall_score_threshold/self.num_class
    #print("scores = ",scores)
    
    json_dict["map"] = mean_ap
    json_dict["precision"] = precisions_score_threshold
    json_dict["recall"] = recall_score_threshold
    json_dict["ap_per_class"] = ap_dict
    json_data = json.dumps(json_dict)
    f = open("./result.json","w")
    f.write(str(json_data))
    f.close()

    print("average_precision_per_class = ",self.average_precision_per_class)
    print("mean_ap = ",mean_ap)
    print("precision: %.4f , recall: %.4f"%(precisions_score_threshold,recall_score_threshold))
