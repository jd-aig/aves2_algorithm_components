...
InputEndpoints = collections.namedtuple(
    'InputEndpoints', ['images', 'images_orig', 'labels', 'labels_one_hot','files'])
...
def central_crop(image, crop_size):
  ...
  with tf.variable_scope('CentralCrop'):
    ...
    image_height, image_width = image.get_shape().as_list()[0], image.get_shape().as_list()[1]
    print(image_height, image_width)
    print(type(image_height))
    print(type(image_width - target_width))
    print(type(image_height - target_height))
    ...
    with tf.control_dependencies([assert_op1, assert_op2]):
      offset_width = int((image_width - target_width) / 2)
      offset_height = int((image_height - target_height) / 2)
      print(image)
...
def get_data(dataset,
  ...
  image_orig, label, file_name = provider.get(['image', 'label','file'])
  ...
  images, images_orig, labels, labels_one_hot ,file_names = (tf.train.shuffle_batch(
      [image, image_orig, label, label_one_hot, file_name],
