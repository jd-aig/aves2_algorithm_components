...
def char_accuracy(predictions, targets, files, rej_char, streaming=False):
  ...
  with tf.variable_scope('CharAccuracy'):
    ...
    #accuracy_per_example = tf.Print(accuracy_per_example,[tf.reduce_sum(weights, 1),tf.reduce_sum(tf.multiply(correct_chars, weights), 1),accuracy_per_example, files],summarize=100000)
    accuracy_per_example = tf.Print(accuracy_per_example, [files, accuracy_per_example],summarize=100000)
    if streaming:
