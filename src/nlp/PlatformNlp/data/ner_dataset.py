...
class NerDataset(BaseDataset):
    ...
    def convert_single_example(self, ex_index, example, label_mapping, tokenizer):
        ...
        label_ids.append(self.label_mapping.get("[CLS]"))
        for i, token in enumerate(tokens):
            ntokens.append(token)
            segment_ids.append(0)
            label_ids.append(self.label_mapping.get(labels[i]))
        ntokens.append("[SEP]")  # 句尾添加[SEP] 标志
        segment_ids.append(0)
        label_ids.append(self.label_mapping.get("[SEP]"))
